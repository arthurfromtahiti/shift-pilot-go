...
## Points d'entrée
- `Remaining(s Slot) int` — `internal/booking/booking.go:24-26`
- `IsAvailable(s Slot) bool` — `internal/booking/booking.go:29-31`
...
1. **L'appelant construit ou obtient un `Slot`** — le `Slot` doit porter des valeurs de `Capacity` (capacité totale) et `Booked` (places déjà réservées) cohérentes. Aucune validation d'entrée n'est effectuée par les fonctions (`internal/booking/booking.go:24-26, 29-31`).

2. **Calcul des places restantes via `Remaining`** — retourne `s.Capacity - s.Booked` (`internal/booking/booking.go:25`). Opération arithmétique pure, sans effet de bord.

3. *(Optionnel, selon le besoin de l'appelant)* **Test de disponibilité via `IsAvailable`** — retourne `Remaining(s) > 0` (`internal/booking/booking.go:30`). `IsAvailable` n'est qu'un prédicat sur le résultat de `Remaining` ; appeler l'une n'empêche pas d'appeler l'autre.
...
- **Places restantes = Capacité − Réservées** : `Remaining(s) = s.Capacity - s.Booked` (`internal/booking/booking.go:25`). La règle est symétrique : un créneau vide (`Booked = 0`) retourne `Capacity` ; un créneau plein retourne `0`.
- **Disponible si et seulement si il reste au moins une place** : `IsAvailable(s) = Remaining(s) > 0` (`internal/booking/booking.go:30`). Un créneau est indisponible dès que `Remaining ≤ 0`.
...
- **`Slot`** (`internal/booking/booking.go:15-21`) : créneau d'activité, support de toutes les opérations. Champs utilisés par ce workflow :
...
- **`Remaining` peut retourner un entier négatif sans avertissement** : si un `Slot` est construit directement avec `Booked > Capacity` (contournant `Book` qui valide cette invariante), `Capacity - Booked < 0`. Aucune garde ni `panic` dans `Remaining` (`internal/booking/booking.go:25`). Impact : l'appelant reçoit un entier négatif sans signal d'erreur — comportement silencieusement incorrect si non anticipé. Note : depuis l'ajout de gardes à `Book` (voir `WORKFLOW_RESERVER_PLACES.md`), la sur-réservation via `Book` est impossible ; ce risque ne s'active que si le `Slot` est contourné.
...
- `internal/booking/booking.go` — lu intégralement (`VÉRIFIÉ_CODE`) : type `Slot` (lignes 15-21), `Remaining` (lignes 24-26), `IsAvailable` (lignes 29-31)
